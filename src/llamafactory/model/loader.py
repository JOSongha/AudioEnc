...
def load_model(
    ...
    if model is None and not lazy_load:
        ...
        if model_args.mixture_of_depths == "load":
            ...
            if type(config) in AutoModelForImageTextToText._model_mapping.keys():  # image-text
                ...
                type(config) in AutoModelForTextToWaveform._model_mapping.keys()
            ):  # audio hack for qwen omni
    ...
    if not lazy_load:
        if is_trainable and model_args.enable_liger_kernel and hasattr(model, "apply_liger_kernel"):
            model.apply_liger_kernel()
            logger.info_rank0("Liger kernel has been applied via model.apply_liger_kernel().")
