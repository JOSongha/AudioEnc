...
# Qwen3.5AE: audio adapter (DAC encoder + Llama-style projector) attached to Qwen3.5 LLM.
# Registering here so Stage-2 LoRA (with `freeze_vision_tower: true` +
# `freeze_multi_modal_projector: true` in yaml) is scoped to language_model only — the
# `patch_target_modules()` filter then excludes anything containing `audio_encoder` /
# `audio_encoder.projector` from LoRA wrapping. The projector stays fully trainable via
# the workflow.py `requires_grad` override (`"audio_encoder.projector" in name`).
_register_composite_model(
    model_type="qwen3_5_ae",
    projector_key="audio_encoder.projector",
