...
def apply_liger_kernel(
    ...
    if model_type == "gemma":
        ...
        from liger_kernel.transformers import (
            apply_liger_kernel_to_gemma3_text as apply_liger_kernel,
        )
    ...
    elif model_type == "qwen3mot":
        # Handled after model loading via instance-level patching in loader.py
        logger.info_rank0("Qwen3MoT liger kernel will be applied after model loading (instance-level patch).")
        return
    elif model_type == "qwen3_5_ae":
        # Handled after model loading via instance-level patching in loader.py
        logger.info_rank0("Qwen3_5AE liger kernel will be applied after model loading (instance-level patch).")
        return
    elif model_type == "qwen3_ae":
        # Handled after model loading via instance-level patching in loader.py
        logger.info_rank0("Qwen3AE liger kernel will be applied after model loading (instance-level patch).")
        return
    elif model_type == "gpt_oss":
        try:
            from liger_kernel.transformers import (
                apply_liger_kernel_to_gpt_oss as apply_liger_kernel,
            )
        except ImportError:
