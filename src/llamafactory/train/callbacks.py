...
from transformers import PreTrainedModel, ProcessorMixin, TrainerCallback, integrations
...
class WandbCallback(integrations.WandbCallback):
    ...
    def setup(self, args, state, model, **kwargs):
        # hijack the wandb.log to fit `step=global_step`
        if state.is_world_process_zero:
            original_log = self._wandb.log
            def wrapped_log(*args, **kwargs):
                kwargs.setdefault("step", state.global_step)
                return original_log(*args, **kwargs)
            self._wandb.log = wrapped_log

        # perform wandb.init hijacking for using the
        # `fork_from` or `resume_from` options:
        # https://docs.wandb.ai/guides/runs/rewind/#fork-from-a-run-that-you-rewind
        if state.is_world_process_zero and args.resume_from_checkpoint:
            if resume_from := os.getenv("WANDB_RESUME_FROM", None):
                logger.info_rank0(f"`WANDB_RESUME_FROM` environment variable "
                                  f"detected. Will resume Wandb log "
                                  f"from '{resume_from}'.")
            elif fork_from := os.getenv("WANDB_FORK_FROM", None):
                logger.info_rank0(f"`WANDB_FORK_FROM` environment variable "
                                  f"detected. Will fork Wandb log "
                                  f"from '{fork_from}'.")

            self._wandb.init = functools.partial(
                self._wandb.init,
                resume_from=resume_from,
                fork_from=None if resume_from else fork_from,
            )

        ret = super().setup(args, state, model, **kwargs)

        if state.is_world_process_zero:
            # Update config
            self._wandb.config['data_args'] = self.data_args
            self._wandb.config['total_batch_size'] = (
            ...
            if getattr(self._wandb, "define_metric", None):
                self._wandb.define_metric("train/token")
                self._wandb.define_metric("*", step_metric="train/token", step_sync=True)

            # Set wandb.alert slack and email integrations in personal user settings
            # to fully take advantage of the function.
            self._wandb.alert(f"Training Started ('{args.run_name}')",
                              pprint.pformat(args, width=80))

        return ret

    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        total_tokens = (args.generation_max_length *
                args.per_device_train_batch_size *
                args.gradient_accumulation_steps *
                args.world_size *
                state.global_step)
        ...
        return super().on_log(args, state, control, model, logs, **kwargs)
