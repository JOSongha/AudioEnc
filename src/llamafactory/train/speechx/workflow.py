...
from ...data import SFTDataCollatorWith4DAttentionMask, get_dataset, get_template_and_fix_tokenizer, get_speechx_dataset
...
from .trainer import SpeechXTrainer
...
def run_speechx(
    ...
    unit_token_ids = [i for v, i in tokenizer.get_vocab().items() if v.startswith("<|audio")]

    template = get_template_and_fix_tokenizer(tokenizer, data_args)
    dataset_module = get_speechx_dataset(template, model_args, data_args, training_args, stage="speechx", **tokenizer_module)
    ...
    data_collator = SFTDataCollatorWith4DAttentionMask(
        ...
        flatten=getattr(model.config, "_attn_implementation")=="flash_attention_2",
    ...
    trainer = SpeechXTrainer(
        ...
        enable_liger_kernel=model_args.enable_liger_kernel,
        **dataset_module,
        **tokenizer_module,
        **metric_module,
    )
