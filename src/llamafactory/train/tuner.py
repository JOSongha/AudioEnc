...
from .callbacks import LogCallback, PissaConvertCallback, ReporterCallback, WandbCallback, ProfilingCallback
...
from .omni import run_omni
from .speechx import run_speechx
...
def update_training_args(training_args, auto_run_name_if_empty=True):
    # if not specified, run_name is output_dir by default from the TrainingArgument
    if training_args.run_name == training_args.output_dir:
        if auto_run_name_if_empty:
            training_args.run_name = datetime.now().strftime(
                '%Y%m%d_%H-%M-%S') if not os.getenv("RUN_NAME", None) else os.getenv("RUN_NAME")
        elif os.getenv("RUN_NAME", None):
            training_args.run_name = os.getenv("RUN_NAME")
        else:
            raise ValueError("Must Specify run_name in config file or `RUN_NAME` in env.")

    training_args.logging_dir = os.path.join(training_args.logging_dir, "runs", training_args.run_name)
    ...
    logger.info_rank0(f"RUN NAME: {training_args.run_name}")
...
def _training_function(config: dict[str, Any]) -> None:
    ...
    training_args = update_training_args(training_args)

    if "wandb" in training_args.report_to:
        callbacks.append(WandbCallback(data_args))
    ...
    elif finetuning_args.stage == "speechx":
        run_speechx(model_args, data_args, training_args, finetuning_args, generating_args, callbacks)          
    ...
    elif finetuning_args.stage == "omni":
        run_omni(model_args, data_args, training_args, finetuning_args, generating_args, callbacks)  
    else:
