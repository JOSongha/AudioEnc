...
register_template(
    name="old_ve",
    format_user=StringFormatter(slots=["다음은 음성을 생성하는 지시문입니다. 주어진 문장인 <|transcript|>의 내용과 구조를 이해한 후, 실제 사람이 발화하는 것처럼 정확하고 자연스럽게 읽어주세요.<|texttospeech|><|transcript|>{{content}}<|endoftext|><|correspond|><|speech|>"]),
    format_assistant=StringFormatter(slots=["{{content}}"]),
