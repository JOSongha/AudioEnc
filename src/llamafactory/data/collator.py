...
def batch_group_counter(batch_tensor, pad_value=0):
    """
    Args:
        batch_tensor: LongTensor of shape (batch_size, seq_len)
        pad_value: Optional, if sequences are padded, specify the pad value.

    Returns:
        group_count_tensor: LongTensor of same shape as batch_tensor,
                            each group counted from 0 within group.

    Example:
        >>> batch_group_counter(torch.tensor([[1, 1, 2, 2, 2, 0]]))
        tensor([[0, 1, 0, 1, 2, 0]])
    """
    batch_size, seq_len = batch_tensor.shape

    # Detect group boundaries
    group_start = torch.ones(batch_size, seq_len, dtype=torch.bool)
    group_start[:, 1:] = batch_tensor[:, 1:] != batch_tensor[:, :-1]

    # Use cummax to find the start position of each group
    arange = torch.arange(seq_len).unsqueeze(0).expand(batch_size, -1)
    start_markers = (arange + 1) * group_start.long()  # +1 to distinguish index 0 from fill
    group_start_pos, _ = torch.cummax(start_markers, dim=1)
    result = arange - (group_start_pos - 1)

    # Zero out padding positions
    if pad_value is not None:
        result = result.masked_fill(batch_tensor == pad_value, 0)
...
class SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: Sequence[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        elif self.block_diag_attn and self.attn_implementation == "flash_attention_2" and self.flatten:
            features['input_ids'] = features['input_ids'][features['attention_mask']!=0].unsqueeze(0)
            features['labels'] = features['labels'][features['attention_mask']!=0].unsqueeze(0)
            features["position_ids"] = batch_group_counter(features["attention_mask"])
            features["position_ids"] = features['position_ids'][features.pop('attention_mask')!=0].unsqueeze(0)
            features['labels'][features['position_ids']==0] = -100
        elif self.block_diag_attn and self.attn_implementation == "flash_attention_2" and not self.flatten:
            features['position_ids'] = batch_group_counter(features.pop('attention_mask'), pad_value=None)
