#!/bin/bash
# Stage1 training: Whisper-tiny v4 (per-modality interleave: asr/sound/emotion 0.65/0.25/0.10).
# Same launcher as DAC-VAE v4; only CONFIG path changes.
...
# crash. token 으로도 안 풀림. cache 만 사용 강제 (v5 12.6k step crash 회귀 방지).
...
echo "[s1-whisper-tiny-v5] launching on $NGPU GPUs"

FORCE_TORCHRUN=1 NPROC_PER_NODE=$NGPU \
    $ENV_PREFIX/bin/llamafactory-cli train configs/ASR/stage1_whisper_tiny_v5.yaml
