#!/bin/bash
# Stage1 training: DAC-VAE v6 (per-modality interleave: asr/sound/emotion 0.65/0.25/0.10).
# Same launcher as v5; only CONFIG path changes (manifest path swap v5 → v6 inside yaml).
...
echo "[s1-dac-vae-v6] launching on $NGPU GPUs"

FORCE_TORCHRUN=1 NPROC_PER_NODE=$NGPU \
    $ENV_PREFIX/bin/llamafactory-cli train configs/ASR/stage1_dac_vae_v6.yaml
