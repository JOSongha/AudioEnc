#!/bin/bash
# Stage1 training: WavTokenizer 40-unify v6 (per-modality interleave: asr/sound/emotion 0.65/0.25/0.10).
# Same launcher as DAC-VAE v6; only CONFIG path changes.
# Requires external/models/Qwen3.5AE-4B-wavtok-projL (run scripts/setup_v6_projL_models.sh first).
...
echo "[s1-wavtok-v6] launching on $NGPU GPUs"

FORCE_TORCHRUN=1 NPROC_PER_NODE=$NGPU \
    $ENV_PREFIX/bin/llamafactory-cli train configs/ASR/stage1_wavtok_v6.yaml
