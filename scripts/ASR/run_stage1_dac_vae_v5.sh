#!/bin/bash
# Stage1 training: DAC-VAE v4 (per-modality interleave: asr/sound/emotion 0.65/0.25/0.10).
# Same launcher as v3; only CONFIG path changes (manifest 분리 + interleave는 yaml 안에서 처리).
...
echo "[s1-dac-vae-v5] launching on $NGPU GPUs"

FORCE_TORCHRUN=1 NPROC_PER_NODE=$NGPU \
    $ENV_PREFIX/bin/llamafactory-cli train configs/ASR/stage1_dac_vae_v5.yaml
