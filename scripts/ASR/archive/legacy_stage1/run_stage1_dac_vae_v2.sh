#!/bin/bash
# Stage1 training: DAC-VAE v2 encoder (jos environment).
# Single-node multi-GPU via torchrun + deepspeed z2.
# Mirror of run_stage1_encodec.sh; differs only in CONFIG path.
...
echo "[s1-dac-vae-v2] launching on $NGPU GPUs"

FORCE_TORCHRUN=1 NPROC_PER_NODE=$NGPU \
    $ENV_PREFIX/bin/llamafactory-cli train configs/ASR/stage1_dac_vae_v2.yaml
