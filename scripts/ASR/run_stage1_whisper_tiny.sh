#!/bin/bash
# Stage1 training: Whisper-tiny encoder (multi-GPU via NSML, deepspeed z2, wandb).
#
# Usage:
#   bash scripts/ASR/run_stage1_whisper_tiny.sh
...
FORCE_TORCHRUN=1 NNODES=$NSML_WORLD_SIZE NODE_RANK=$NSML_RANK MASTER_ADDR=$NSML_HOST_RANK0 MASTER_PORT=21267 \
    llamafactory-cli train /mnt/ddn/users/sehyun/AudioEncoder/audiollm-trainer/configs/ASR/stage1_whisper_tiny.yaml
