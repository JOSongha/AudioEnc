#!/bin/bash
# Stage1 training: Whisper-small v6 (per-modality interleave: asr/sound/emotion 0.65/0.25/0.10).
...
# crash. token 으로도 안 풀림. cache 만 사용 강제 (v5 의 12.6k step crash 와 동일).
...
echo "[s1-whisper-small-v6] launching on $NGPU GPUs"

FORCE_TORCHRUN=1 NPROC_PER_NODE=$NGPU \
    $ENV_PREFIX/bin/llamafactory-cli train configs/ASR/stage1_whisper_small_v6.yaml
