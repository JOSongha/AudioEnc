#!/bin/bash
# Stage2 resume: WavTok-40-unify lora64, from checkpoint-4000.
...
# Default collective timeout is 600s — bump to 30 min so a slow nubes download
# on one rank doesn't kill the whole job.
export TORCH_NCCL_TIMEOUT_SEC=1800
export NCCL_TIMEOUT=1800000  # ms
export TOKENIZERS_PARALLELISM=false
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
export OMP_NUM_THREADS=2
export MKL_NUM_THREADS=2

# Redirect caches off ~ (machine dies if home > 30 GB).
...
FORCE_TORCHRUN=1 NNODES=$NSML_WORLD_SIZE NODE_RANK=$NSML_RANK MASTER_ADDR=$NSML_HOST_RANK0 MASTER_PORT=21267 \
    ...
    /mnt/ddn/users/sehyun/AudioEncoder/audiollm-trainer/configs/qwen3_5ae-asr/stage2_wavtok_40_unify_lora64_resume.yaml
