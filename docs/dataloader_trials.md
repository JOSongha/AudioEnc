...
| **§43 Qwen3 family 분기 지원** | `--llm Qwen/Qwen3-1.7B` 등 Qwen3 (plain transformer) 지원. Liger (`apply_liger_kernel_to_qwen3`) / FSDP wrap cls (`Qwen3DecoderLayer`) / wandb 라벨 (`Qwen3-1.7b-ASR-<enc>`) 을 `_qwen_family` 헬퍼로 분기. `<\|image_pad\|>=151655` 은 Qwen3 에서도 그대로라 `audio_pad_token_id` 재사용 OK. | ✅ 구현 완료 |
| **§43 packed bin in-place 재토크나이징** | `--skip-mixed-pack` runtime repack 느림 (ls100 single-shard, 첫 step 수 분 대기) + full offline re-pack 은 encoder 도 다시 안 돌리지만 역시 수십 분. 대안: **기존 packed bin 에서 input_ids / labels 만 src_tok → dst_tok 로 swap** (audio_features / audio_lengths 는 그대로). `retokenize_packed.py` 신규 스크립트 — row-by-row streaming (RSS ~650 MB/rank), 8 rank 병렬로 **~12 분** 만에 868 GB packed 전환. | ✅ rank 0 decode 검증 OK, 8-rank 전체 완료, Stage 1 smoke loss 6.34→4.32 (step 51) 정상 | 
...
| `config.py:_infer_llm_family` / `_infer_llm_tag` | §43 헬퍼 추가 — `cfg["llm_model"]` 문자열에서 family (`Qwen3.5` / `Qwen3`) 와 size tag (`2b` / `1.7b` / `4b` …) 를 정규식으로 추출. `project_name = f"{family}-{tag}-ASR-{encoder}"` 로 wandb 라벨 재구성. 기존 `"2b" if "2B" in llm_model else "4b"` 는 1.7B 을 "4b" 로 오분류. |
| `config.py:get_config` | §43 `llm_tag` / `project_name` 산출을 새 헬퍼로 교체. 기본값 (`Qwen/Qwen3.5-2B`) 는 그대로 유지해 기존 Qwen3.5 런은 동일 project 로 떨어짐. |
| `train_pipeline_override.py:_qwen_family` / `_fsdp_wrap_cls_for` | §43 AudioQwen 위에 헬퍼 추가 — `_qwen_family("Qwen/Qwen3-1.7B") == "qwen3"` / `"Qwen/Qwen3.5-2B" == "qwen3_5"`. FSDP `fsdp_transformer_layer_cls_to_wrap` 을 `["Qwen3_5DecoderLayer"]` 하드코딩에서 `_fsdp_wrap_cls_for(cfg["llm_model"])` 로 치환 (Stage 1 / Stage 2 두 곳). |
| `train_pipeline_override.py:AudioQwen.__init__` | §43 Liger kernel 분기 — `_qwen_family == "qwen3_5"` 는 기존 partial-RoPE / RMSNorm offset / SwiGLU 수동 패치 경로 유지. `"qwen3"` 는 liger-kernel 공식 `apply_liger_kernel_to_qwen3(rope, rms_norm, swiglu, fused_linear_cross_entropy)` 한 줄로 처리 (Qwen3 은 full-RoPE + 표준 RMSNorm). 그 외 family 는 warn+skip. |
| `train_pipeline_override.py:main()` | §43 `--llm` 오버라이드 시 `cfg["project_name"]` 재계산 로직 추가. `get_config` 가 TRAIN_CONFIG 기본값 기준으로 project_name 을 먼저 박아놔서, 오버라이드만 하면 wandb 에는 여전히 옛 라벨이 찍히던 버그. `llm_tag` 계산도 `_infer_llm_tag` helper 로 통일. |
| `docs/dataloader_trials.md` | §43 본 문서에 Qwen3 family 지원 row 추가 + 파일 변경 이력 섹션에 5 개 row. 실행 플래그: `--llm Qwen/Qwen3-1.7B --skip-mixed-pack --precomputed-dir <dir>` (mixed packed arrows 는 Qwen3.5 토크나이저로 생성돼 vocab OOB 발생 — per-dataset rank*.arrow 에는 raw `text` 가 들어있어 런타임 re-tokenize 로 우회). |
| `arXiv/scripts/precompute/retokenize_packed.py` (신규) | §43 in-place 재토크나이징 스크립트. `pa.memory_map` + per-row slice → 1-row RecordBatch 를 writer 에 즉시 flush (streaming). batch 전체 `audio_features` 를 `.to_pylist()` 하거나 `pc.take` 로 복사하면 RSS 20 GB/rank 로 폭증 → row-by-row slice 로 **650 MB/rank** 로 억제. tqdm 으로 bin 진행률 표시. sample chunk 분류 = `(is_audio_pad, is_labeled)` 튜플 변화 → src_tok.decode(특수토큰 보존) → dst_tok.encode. eos/pad 같은 special token 도 string roundtrip 으로 올바르게 ID 교체 (Qwen3.5 `<\|im_end\|>=248046` → Qwen3 `<\|im_end\|>=151645`). |
| `arXiv/scripts/precompute/run_retokenize.sh` (신규) | §43 8-rank 병렬 wrapper. `--src-dir` + `--src-llm` + `--dst-llm` 만 받아서 rank 당 python 프로세스 fork. fb_dacvae ls100+ls360+ls500+mls+gs+vp 전체 (64 shards × ~7500 bin/rank) **~12 분** 완료. |
| `train_pipeline_override.py:_llm_pack_suffix` / `build_precomputed_pipeline` / `count_total_precomputed_bins_per_rank` | §43 packed dir 조회에 llm suffix 우선. `Qwen/Qwen3-1.7B` 시 `packed_half_inlv_{N}_qwen3_1.7b/` 를 먼저 찾고, 없으면 기존 suffix 없는 dir 로 fallback. 기본 `Qwen3.5-2B` 는 기존 경로 그대로 (backwards compat). |
| `arXiv/scripts/precompute/half_inlv_pack_arrow.py` / `run_half_inlv_pack.sh` | §43 `--llm` arg 추가 (향후 offline re-pack 필요 시 바로 사용 가능). 출력 subdir 에 `{llm_family}_{tag}` suffix 자동 적용. 지금은 retokenize 경로가 훨씬 빨라서 사용 안 함. |
