...
> **제외**: CREMA-D (path 가 다른 user 소유, 영구 제외), IEMOCAP Session 5 (예외 — 학계 관행 leave-session-out eval, 2,170 utt held-out), MELD test split (공식 train/dev/test 의 test, Stage-2 eval 과 일치).
...
Stage-2 eval ([`evaluation/stage2/`](../../evaluation/stage2/)) 도 nubes-aware patch 적용 — `EVAL_USE_NUBES=1` env 활성 시 nubes 게이트웨이에서 audio / metadata fetch. helper: [`_nubes_loader.py`](../../evaluation/stage2/_nubes_loader.py).

Patched eval scripts (5):
...
- `eval_esc50_acc.py` — audio + esc50.csv
- `eval_clotho_caption.py` — development split (eval / val 은 nubes 부재라 local fallback)

미패치 (nubes 미업로드 또는 다중 corpus 복잡): `eval_source_emotion.py`, `eval_librispeech_wer.py`, `eval_msp_podcast.py`, `eval_savee.py`, `eval_jl_corpus.py`, `eval_listen_official.py`.
