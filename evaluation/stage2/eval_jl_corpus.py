"""JL-Corpus emotion eval (NZ English, 7 secondary emotions).

Emotions: angry / anxious / apologetic / assertive / concerned / encouraging / excited
4 speakers (2 female + 2 male), prompted speech.
NOT in any training pool.

Source: CLAPv2/JL-Corpus HF mirror (30 parquets). Filename has emotion encoded:
    (female|male)\\d_<emotion>_<sentence_id>_<repetition>

Usage:
    python -m evaluation.stage2.eval_jl_corpus --ckpt-root ... --out-root ... --ckpts ...
"""
...
CACHE_ROOT = Path("/mnt/tmp/cache/jl_corpus")
LABELS = ["angry", "anxious", "apologetic", "assertive", "concerned", "encouraging", "excited"]
...
NAME_RE = re.compile(r"(?:female|male)\d_([a-z]+)_")


def load_jl() -> list[dict]:
    from huggingface_hub import HfApi, hf_hub_download
    import pyarrow.parquet as pq
    api = HfApi()
    files = sorted(f for f in api.list_repo_files('CLAPv2/JL-Corpus', repo_type='dataset') if f.endswith('.parquet'))
    print(f"[jl] {len(files)} parquets")
    rows = []
    for f in files:
        p = hf_hub_download(repo_id='CLAPv2/JL-Corpus', repo_type='dataset',
                            filename=f, local_dir=str(CACHE_ROOT))
        t = pq.read_table(p)
        df = t.to_pandas()
        for _, r in df.iterrows():
            idx = r["index"]
            m = NAME_RE.match(str(idx))
            if not m:
                continue
            emo = m.group(1)
    ...
    print(f"[jl] loaded {len(rows)}  dist: {dict(Counter(r['gold_label'] for r in rows))}")
    return rows
...
def run_eval(model, tokenizer, cfg, rows, *, batch_size, max_audio_samples, max_new_tokens):
    ...
    for start in range(0, len(prepared), batch_size):
        ...
        if (start // batch_size) % 20 == 0:
            ...
            print(f"[jl] {done}/{len(prepared)}  {rate:.2f} sps  eta {eta:.0f}s", flush=True)
    return preds
...
def main():
    ...
    print(f"[jl] {len(ckpts)} checkpoint(s)", flush=True)
    for ckpt_path in ckpts:
        out_dir = out_root / ckpt_path.name
        sum_path = out_dir / "summary.json"
        if sum_path.exists():
            print(f"[jl] skip {ckpt_path.name}", flush=True); continue
        out_dir.mkdir(parents=True, exist_ok=True)
        print(f"[jl] loading {ckpt_path}", flush=True)
        ...
        print(f"[jl] {ckpt_path.name}  acc={s['accuracy']:.4f}  macro-F1={s['macro_f1']:.4f}  WA={s['weighted_accuracy']:.4f}  n={s['n_total']}", flush=True)
