"""Whisper encoder (model_id from config) + 4-layer Llama causal projector audio encoder.
...
class AudioEncoder(nn.Module):
    ...
    def forward(self, audio_features, use_cache=False):
        ...
        # autocast promotes the encoder's final LayerNorm to fp32, so its output is fp32 even
        # though encoder weights are bf16. Recast to projector dtype before the trainable matmul.
        hidden = hidden.to(self.projector.input_proj.weight.dtype)
