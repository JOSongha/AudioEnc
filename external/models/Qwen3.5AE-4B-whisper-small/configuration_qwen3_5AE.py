...
class AudioConfig(PretrainedConfig):
    r"""
    Configuration class for the audio adapter module (Whisper-small.en encoder variant).

    Defines a causal transformer adapter based on Llama decoder layers that connects the
    Whisper encoder output (last_hidden_state) to the LLM.
    """
    ...
    def __init__(
        self,
        audio_hidden_size=768,           # Whisper-small d_model
        ...
        whisper_fps=50,                  # encoder output frame rate (after 2x conv stride)
        whisper_max_frames=1500,         # 30s × 50fps
        whisper_hop_length=320,          # samples per output frame (16000 / 50)
        **kwargs,
    ):
        ...
        self.whisper_fps = whisper_fps
        self.whisper_max_frames = whisper_max_frames
        self.whisper_hop_length = whisper_hop_length
